...
namespace seam {
	...
	private:
		struct OctreeNode {
			...
			const std::array<OctreeNode*, 8>& BranchChildren() {
			...
			void AddBranchChild(uint8_t index, OctreeNode* child) {
				assert(is_branch);
				assert(branch_children[index] == nullptr);
				branch_children[index] = child;
				child->parent = this;
			...
			OctreeNode* RemoveBranchChild(uint8_t index) {
				assert(is_branch);
				assert(branch_children[index] != nullptr);
				OctreeNode* child = branch_children[index];
				assert(child->parent == this);
				child->parent = nullptr;
				branch_children[index] = nullptr;
		...
		void Add(T* item, glm::vec3 position, OctreeNode* branch, glm::vec3 center, glm::vec3 bounds) {
			...
			assert(res.branch->IsBranch());
			OctreeNode* leaf = res.leaf;
			if (res.leaf == nullptr) {
				// TODO replace me with grabbing a leaf from a pool
				leaf = new OctreeNode(false, res.branch);
				res.branch->AddBranchChild(res.leaf_index, leaf);
		...
		void SplitLeaf(OctreeNode* branch, OctreeNode* leaf, glm::vec3 center, glm::vec3 bounds) {
			for (size_t i = 0; i < leaf->count; i++) {
				...
				OctreeNode* new_leaf = branch->BranchChildren()[leaf_index];
				if (new_leaf == nullptr) {
					// TODO POOL POOOL
					new_leaf = new OctreeNode(false, branch);

					// branch->branch_children[leaf_index] = 
				}
